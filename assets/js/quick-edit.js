/**
 * Populate rareview_scheduled_sticky_posts quick edit field from row data.
 */
...
document.addEventListener( 'DOMContentLoaded', () => {
	...
	inlineEditPost.edit = function ( id ) {
		...
		const dataEl = postRow?.querySelector(
			'.rareview-scheduled-sticky-posts-quick-edit-data'
		);
		const isStickified =
			1 === Number( dataEl?.dataset.rareviewScheduledStickyPosts );

		const editRow = document.getElementById( `edit-${ postId }` );

		const touched = editRow?.querySelector(
			'input[name="rareview_scheduled_sticky_posts_quick_edit_touched"]'
		);
		if ( touched ) {
			touched.value = '0';
		}
		...
		const checkbox = editRow?.querySelector(
			'input[name="rareview_scheduled_sticky_posts_quick_edit"]'
		);
		if ( checkbox ) {
